...
public class ShellViewModel : Conductor<IScreen>.Collection.OneActive
{
    ...
    public ShellViewModel(IWindowManager windowManager, IConfigurationRoot configuration, D2RLauncherIntegration syncIntegration) //Main Window Logger
    {
        _windowManager = windowManager;
        _configuration = configuration;
        _syncIntegration = syncIntegration;
    ...
    public async void OnLoaded(object args) //Functions to perform after UI has been loaded
    {
        ...
        // Initialize D2GitSync integration
        await InitializeSyncServiceAsync();
    ...
    public async Task ApplyTCPPatch()
    {
        ...
        // Start D2GitSync before launching the game
        await OnGameStartingAsync();
        ...
        if (process != null)
        {
            ...
            // Monitor the game process for exit in background
            _ = Task.Run(async () => await MonitorGameProcessAsync(process));
        }

        else
        {
            _logger.Error("Failed to launch the process.");
            // Stop sync service if game failed to start
    ...
    #region ---D2GitSync Integration---

    private async Task InitializeSyncServiceAsync()
    {
        try
        {
            if (_syncIntegration != null)
            {
                _logger.Info("Initializing D2GitSync service...");
                
                // Load configuration from file or create default
                var configPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "D2RLAN", "d2gitsync.json");
                SyncConfiguration config;
                
                if (File.Exists(configPath))
                {
                    config = SyncConfiguration.LoadFromFile(configPath);
                    _logger.Info("Loaded D2GitSync configuration from file");
                }
                else
                {
                ...
                else
                {
                    ...
                    _logger.Info("D2GitSync service initialized but disabled (no remote repository configured)");
                }
                
                // Subscribe to sync events for status updates
                _syncIntegration.StatusChanged += OnSyncStatusChanged;
                _syncIntegration.ErrorOccurred += OnSyncErrorOccurred;
                
                _logger.Info("D2GitSync service initialization completed");
            }
        }
        catch (Exception ex)
        {
            _logger.Error($"Failed to initialize D2GitSync service: {ex.Message}");
        }
    }

    public async Task OnGameStartingAsync()
    {
        try
        {
            if (_syncIntegration != null)
            {
                ...
                _logger.Info($"Starting D2GitSync for saves path: {savesPath}");
                await _syncIntegration.OnGameStartingAsync(savesPath);
            }
        }
        catch (Exception ex)
        {
            _logger.Error($"Error starting D2GitSync: {ex.Message}");
        }
    }
    ...
    public async Task OnGameEndedAsync()
    {
        try
        {
            if (_syncIntegration != null)
            {
                _logger.Info("Stopping D2GitSync service");
                await _syncIntegration.OnGameEndedAsync();
            }
        }
        catch (Exception ex)
        {
            _logger.Error($"Error stopping D2GitSync: {ex.Message}");
        }
    }

    private string GetSavesPath()
    {
        // D2RLAN already knows where saves are - we can use their logic
        // Default path that D2RLAN uses
        string defaultPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Saved Games", "Diablo II Resurrected");
        ...
        string documentsPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Diablo II Resurrected");
        if (Directory.Exists(documentsPath))
            return documentsPath;
            
        // Return default path even if it doesn't exist - user might create it
        return defaultPath;
    }

    private void OnSyncStatusChanged(object sender, SyncStatusEventArgs e)
    {
        // Log status changes for now - could be enhanced with UI notifications
        _logger.Info($"Sync Status: {e.Status} - {e.Message}");
    }

    private void OnSyncErrorOccurred(object sender, SyncErrorEventArgs e)
    {
        _logger.Error($"Sync Error: {e.Message}");
        if (e.Exception != null)
        {
            _logger.Error($"Exception: {e.Exception}");
        }
    }

    public D2RLauncherIntegration GetSyncIntegration()
    {
        return _syncIntegration;
    }

    private async Task MonitorGameProcessAsync(Process gameProcess)
    {
        try
        {
            if (gameProcess != null)
            {
                _logger.Info($"Monitoring game process {gameProcess.Id} for exit...");
                
                // Wait for the process to exit
                await gameProcess.WaitForExitAsync();
                
                _logger.Info("Game process has exited, stopping sync service");
                await OnGameEndedAsync();
            }
        }
        catch (Exception ex)
        {
            _logger.Error($"Error monitoring game process: {ex.Message}");
            // Still try to stop the sync service
            await OnGameEndedAsync();
        }
    }
